...
#else /* __iset__ < 5 */
  cy_limb = prod_high + __builtin_e2k_subcd_c (x, prod_low, 0);
#endif /* __iset__ < 5 */
  prod_low = x - prod_low;
  res_ptr[0] = prod_low;

  if (1 < s1_size) {           /* откручена вторая итерация */
      x = res_ptr[1];
      ...
      if (prod_low > x) cy_limb++;
      prod_low = x - prod_low;
#else /* __iset__ < 5 */
      int carry1 = __builtin_e2k_addcd_c (prod_low, cy_limb, 0);
      prod_low += cy_limb;
      int carry2 = __builtin_e2k_subcd_c (x, prod_low, 0);
      prod_low = x - prod_low;
