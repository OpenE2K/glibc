...
%  ГИПЕРБОЛИЧЕСКИЙ' КОМПЛЕКСНОГО  АРГУМЕНТА.             %
%  НЕДОПУСТИМЫМ  ЗНАЧЕНИЕМ  АРГУМЕНТА  ЯВЛЯЕТСЯ          %
%  RE > 710.4758600739439771.                            %
%                                                        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
...
__ccosf (__complex__ float x)
{
    ...
    if (__glibc_unlikely (absim.int0 >= 0x7f800000)) { /* |im| == Inf or Nan */
        __real__ res = NaN_F;
        __imag__ res = (absre.int0 == 0) ? 0 : NaN_F;
        return res;
    }
...
#pragma asm_inline
    ...
    psi.llong += (LL) signre << 32;    /* sin(im) with sinh(re)'s sign */
...
//#if __iset__ >= 6
    ...
    __real__ res = (FL) (exp.value * pco.value);   /* cos(im)*cosh(re) */
    __imag__ res = (FL) (exm.value * psi.value);   /* sin(im)*sinh(re) */
    return res;
}
