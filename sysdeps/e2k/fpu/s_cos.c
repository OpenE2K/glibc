...
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                        %
%     ПРОГРАММА  ВЫЧИСЛЯЕТ  ЗНАЧЕНИЕ  ФУНКЦИИ  'КОСИНУС'.%
%  АРГУМЕНТ  В  РАДИАНАХ.                                %
%                                                        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
...
#if 0
static DB CONSTD[] = {
    1.5894736651849095247744e-10,       /* KA13 */
    -2.5050716974102745027602e-08,      /* KA11 */
    2.7557313376400129128204e-06,       /* KA9  */
    -1.9841269828650300013059e-04,      /* KA7  */
    8.3333333333203624567193e-03,       /* KA5  */
    -1.1360687302437253587387e-11,      /* KA14 */
    2.0875740157060361726804e-09,       /* KA12 */
    -2.7557314436057753118620e-07,      /* KA10 */
    2.4801587289509923185682e-05,       /* KA8  */
    -1.3888888888873247734608e-03,      /* KA6  */
    4.1666666666666577342883e-02,       /* KA4  */
    6.75539944105574400e+15,            /* RS = 3*2^51 = 0x4338000000000000 */
};
    DB KA13 = CONSTD[0];          /* 1.5894736651849095247744e-10     */
    DB KA11 = CONSTD[1];          /* -2.5050716974102745027602e-08    */
    DB KA9 = CONSTD[2];           /* 2.7557313376400129128204e-06     */
    DB KA7 = CONSTD[3];           /* -1.9841269828650300013059e-04    */
    DB KA5 = CONSTD[4];           /* 8.3333333333203624567193e-03     */
    DB KA14 = CONSTD[5];          /* -1.1360687302437253587387e-11    */
    DB KA12 = CONSTD[6];          /* 2.0875740157060361726804e-09     */
    DB KA10 = CONSTD[7];          /* -2.7557314436057753118620e-07    */
    DB KA8 = CONSTD[8];           /* 2.4801587289509923185682e-05     */
    DB KA6 = CONSTD[9];           /* -1.3888888888873247734608e-03    */
    DB KA4 = CONSTD[10];          /* 4.1666666666666577342883e-02     */
    DB RS = CONSTD[11];           /* 6.75539944105574400e+15 = 3*2^51 */
...
static LD CONSTLD[] = {
    ...
    DB KA0 = CONSTD[12];     /* 1.0000000000000000000000e+00     */
...
#pragma asm_inline
    ...
    if (k0 & 1) {
        ...
        lres1.llong = __builtin_e2k_fmad (lres1.llong, lyv4.llong, lres.llong);
        lres.llong = __builtin_e2k_fmad (lres1.llong, lyv8.llong, lres2.llong);
        darg0.db = (DB) ((KA0 + ldxv2 * KA2) + lres.value);
    }
    else {
        lyv8.value = X * xv4;
    ...
    if (k0 & 1) {
        ...
        darg0.db = (DB) ((KA1 + KA3 * ldxv2) * ldx + (KA7 * X * xv4 * (KA5 / KA7 + xv2) +
            X * xv8 * (KA9 + KA11 * xv2 + KA13 * xv4)));
    }
#endif /* __iset__ < 6 */
    darg0.ll = __builtin_e2k_paddd (darg0.ll, ((LL)(k0 & 2)) << 62);
    return darg0.db;
}
