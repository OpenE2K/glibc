...
//        p = KA2 * z * (KA1/KA2 + z) + KA3 * z * zv2 + zv4 * (KA4 +
    ...
    if (arg.twolong.int0 >= 0x3ff00000)
        return (arg.value > 1.0) ? SVID_libm_err(X, 0.0, 2)
            : X / arg.value * PION2;  /* NaN ÉÌÉ ðé/2*sign(X) */
...
#else /* __iset__ < 6 */
// ðÏÓÌÅÄÎÑÑ ÞÁÓÔØ ÐÏÌÉÎÏÍÁ ×ÙÞÉÓÌÑÅÔÓÑ ÐÏÓÌÅÄÏ×ÁÔÅÌØÎÏ ÄÌÑ ÂÏÌØÛÅÊ ÔÏÞÎÏÓÔÉ.
// ÷ÓÅ ÒÁ×ÎÏ ÅÓÔØ ÚÁÐÁÓ ×ÒÅÍÅÎÉ ÄÏ ÏËÏÎÞÁÎÉÑ sqrt É fdivd.
