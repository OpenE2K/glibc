...
#define PIO2_1  1.57079632679233327508e+00  /* первые 36 бит pi/2 = 0x3ff921fb54440000 */
#define PIO2_2  2.56334415158395578912e-12  /* вторые 36 бит pi/2 = 0x3d868c234c4c0000 */
#define PIO2_2T 1.05629990669874271122e-23L /* оставшиеся 64 бит pi/2 = 3fb2 0xcc51701b839a2520 */
...
LD __cosl (LD X)
{
    ...
    if (X <= (PION2 / 2)) {     /* |x| <= pi/4 */
        xv2 = X * X;
        xv4 = xv2 * xv2;
        return KC8 * xv2 * xv4 * xv4 * (KC5 / KC8 + KC6 / KC8 * xv2 + xv4 * (KC7 / KC8 + xv2)) +
            ((KC4 * xv2 + KC3) * xv2 + KC2) * xv4 - 0.5 * xv2 + 1;
    }
    if (X > 8.43314856532626152039e8) { /* |x| > 2^28 * pi = 0x41c921fb54442d18 */
        y0 = __rem_pio2l (X, &y1, &k);
        X = y0 + (LD) y1;
        xl = y0 - X + y1;
    }
    else {     /* |x| > pi/4, но |x| <= 2^28 * pi */
//        arg.value = X * K2ONPI + RS;
//        k = arg.twolong.long1;
//        fn0 = arg.value - RS;
        arg.value = X * K2ONPI + 0.5;
        k = (int) arg.value;
        fn0 = (LD) k;
        a0 = X - fn0 * PIO2_1;
        w0 = fn0 * PIO2_2;
    ...
    xv4 = xv2 * xv2;
    if (k & 1) { /* sin */
        arg.value = KS7 * X * xv4 * xv4 * (KS4 / KS7 + KS5 / KS7 * xv2 + xv4 * (KS6 / KS7 + xv2)) +
            xl + ((xv2 * KS3 + KS2) * xv2 + KS1) * (xv2 * X) + X;
    }
    else { /* cos */
        arg.value = KC8 * xv2 * xv4 * xv4 * (KC5 / KC8 + KC6 / KC8 * xv2 + xv4 * (KC7 / KC8 + xv2)) +
            X * xl + ((KC4 * xv2 + KC3) * xv2 + KC2) * xv4 - 0.5 * xv2 + 1;
    }
    k++;
    if (k & 2) arg.value = -arg.value;
    return arg.value;
}
