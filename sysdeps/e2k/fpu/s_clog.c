...
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                        %
%     ПРОГРАММА  ВЫЧИСЛЯЕТ  НАТУРАЛЬНЫЙ  ЛОГАРИФМ  КОМП- %
%  ЛЕКСНОГО  ЧИСЛА.  НЕДОПУСТИМЫМИ  ЗНАЧЕНИЯМИ  АРГУ-    %
%  МЕНТОВ  ЯВЛЯЮТСЯ  RE = 0  И  IM = 0.                  %
%                                                        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
...
__clog (__complex__ double x)
{
    ...
    if (__glibc_unlikely (absre.llong > 0x7ff0000000000000LL ||
                          absim.llong > 0x7ff0000000000000LL)) { /* |re| или |im| == Nan */
        __imag__ res = NaN;
   ...
   /* вычисление max=Mmax^2+min^2*2^-Пmax и exp=2*Пmax */
    exp = -1022;
    if (max.value - min.value == max.value) {
        ...
        if (max.llong >= 0x0010000000000000LL)
            ...
            max.value *= DVAIN1022;     /* денормализованный max */
    }
    else {
        re = 0.5;
        exp = -2044;
        if (min.llong < 0x0010000000000000LL) {
            max.value *= DVAIN1022;     /* денормализованный min */
            min.value *= DVAIN1022;
        }
        else {
        ...
        if (b < min.value) {    /* близкие max и min */
            arg1.llong = max.llong & ~0xfffffffLL; /* 25 и 28 разрядов */
            y = max.value - arg1.value;
            arg2.llong = min.llong & ~0xfffffffLL; /* 25 и 28 разрядов */
