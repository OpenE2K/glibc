...
%  КОМПЛЕКСНОГО АРГУМЕНТА. НЕДОПУСТИМЫМ ЗНАЧЕНИЕМ        %
%  АРГУМЕНТА ЯВЛЯЕТСЯ   IM > 710.4758600739439771.       %
%                                                        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
...
__csin (__complex__ double x)
{
    ...
    if (__glibc_unlikely (absim.llong >= 0x7ff0000000000000LL)) { /* |im| == Inf or Nan */
        if (absim.llong == 0x7ff0000000000000LL) { /* |im| == Inf */
            ...
            if (absre.llong == 0) {
                ...
                __sincos (absre.value, &sinix.value, &cosix.value);
                sinix.llong &= 0x8000000000000000LL;
                cosix.llong &= 0x8000000000000000LL;
                sinix.llong += 0x7ff0000000000000LL + signre;
                cosix.llong += 0x7ff0000000000000LL + signim;
                __real__ res = sinix.value;
                __imag__ res = cosix.value;
            }
            else {
                __real__ res = NaN;
                __imag__ res = Inf;
            ...
            __real__ res = (absre.llong == 0) ? re : NaN;
            __imag__ res = NaN;
        }
        return res;
    }
    ...
    /* вычисление si=sin(re) и co=cos(re) */
    darg1.db = re * TWOONPI;
//FIXME: при использовании fdtoifd без переделки - знак -0 меняется на +0
#if __iset__ <= 2
    darg1.db += RS;
    t0 = darg1.db - RS;
    k0 = darg1.ll;
...
#pragma asm_inline
...
#else /* __iset__ < 6 */
    ldx = t0 * (LD) PIO2_1 + (LD) re - t0 * PIO2_1T;
#endif /* __iset__ < 6 */
    if (unlikely(absre.llong > 0x40d921fb00000000LL)) {      /* 2^13*pi */
        y = __rem_pio2 (re, &y0, &k1);
        ldx = (LD) y + (LD) y0;
        k0 = k1;
    }
    else {
    ...
    si.ll += ((long long)(k0 & 2)) << 62;
    co.ll += ((long long)((k0 + 1) & 2)) << 62;

    /* вычисление exm=sinh(im) и exp=cosh(im) */
    co.ll += signim;    /* cos(re) with sinh(im)'s sign */
    y = absim.value * K256ONLN2;
    k = (LL) y;

    if (absim.value > (3 * DB_MAX_LOG)) {
        /* Overflow (original image part of x > 3max).  */
        k = 1023 * 3;
    }
    else {
        km = -k;
...
#pragma asm_inline
    __asm ("fdtoifd 0x3,%1,%0" : "=r" (y): "r" (y));
...
#if __iset__ >= 6
        ly.value = y;
        lres.llong = __builtin_e2k_fnmad (LKLN2ON256_hi, ly.llong, absim.llong);
        y = lres.value - y * KLN2ON256_lo;
#else /* __iset__ < 6 */
        y = absim.value - y * KLN2ON256_hi - y * KLN2ON256_lo;
#endif /* __iset__ < 6 */
        yv2 = y * y;
#if __iset__ >= 6
        ly.value = y;
        ...
        if (__glibc_likely (k >= -1021 && k <= 1024)) {
...
#endif /* __iset__ < 6 */
            ym = exp.value - exm.value + arg.value; /* sinh(im) */

            yv2 = im * im;
            yp = yv2 * yv2;

            if (absim.llong < 0x3fe6666666666666LL) {        /* cosh for |im| < 0.7 */
#if __iset__ >= 6
                lyv2.value = yv2;
                lyv4.value = yp;
                lyv8.value = yp * yp;
            ...
            if (absim.llong < 0x3fecccccccccccccLL) {        /* sinh for |im| < 0.9 */
#if __iset__ >= 6
                lyv2.value = yv2;
                lyv4.value = yp;
                lyv8.value = yp * yp;
                ...
                lres1.llong = __builtin_e2k_fmad (lres.llong, lyv8.llong, lres1.llong);
                lres.value = absim.value * yv2;
                exm.llong = __builtin_e2k_fmad (lres1.llong, lres.llong, absim.llong);
#else /* __iset__ < 6 */
                exm.value = KD5 * absim.value * yv2 * (KD3 / KD5 + yv2) + KD11 * absim.value * yv2 * yp * (yv2 * (KD9 / KD11
                    + yv2) + KD7 / KD11) + absim.value * yp * (yp * yp) * (KD15 * yv2 + KD13) + absim.value;
#endif /* __iset__ < 6 */
            }
            else exm.value = ym;
            __real__ res = exp.value * si.db;   /* sin(re)*cosh(im) */
            __imag__ res = exm.value * co.db;   /* cos(re)*sinh(im) */
            return res;
        }

        if (k > 1024) { /* для предотвращения потери точности при очень маленьких sin(re) */
            si.db *= DVAIN1022;
            co.db *= DVAIN1022;
            k -= 1022;
        }
        k = k - 1;
        exp.value = exp.value + arg.value; /* cosh(im) == sinh(im) */
        si.db = exp.value * si.db;   /* sin(re)*cosh(im) */
        co.db = exp.value * co.db;   /* cos(re)*sinh(im) */
    }
    __real__ res = __scalbn (si.db, k);
    __imag__ res = __scalbn (co.db, k);
    return res;
}
