...
%  'áòëóéîõó'.  îåäïðõóôéíùí   úîáþåîéåí   áòçõíåîôá     %
...
FL __asinf (FL X)
{
	...
	yv2 = z * z;
        ...
        lres.llong = __builtin_e2k_fmad (lres1.llong, lzv8.llong, lres2.llong);
        return (FL) lres.value;
#else /* __iset__ < 6 */
        return (FL) (z + KA5 * z * yv2 * (KA3/KA5 + yv2) + KA13 * z * yv2 * yv4 *
    ...
    if (arg.value > 1) {    /* |X|>1 */
        return (FL) SVID_libm_err (z, 0, 102);
    }
...
strong_alias (__asinf, __asinf_finite)
weak_alias (__asinf, asinf)
weak_alias (__asinf, __ieee754_asinf)
