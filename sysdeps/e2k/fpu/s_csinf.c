...
__csinf (__complex__ float x)
{
    ...
    if (__glibc_unlikely (absim.int0 >= 0x7f800000)) { /* |im| == Inf or Nan */
        if (absim.int0 == 0x7f800000) { /* |im| == Inf */
            ...
            if (absre.int0 == 0) {
                ...
                __sincosf (absre.value, &sinix.value, &cosix.value);
                sinix.int0 &= 0x80000000;
                cosix.int0 &= 0x80000000;
                sinix.int0 += 0x7f800000 + signre;
                cosix.int0 += 0x7f800000 + signim;
            ...
            __real__ res = (absre.int0 == 0) ? re : NaN_F;
            __imag__ res = NaN_F;
        }
        return res;
    }
...
#pragma asm_inline
    ...
    psi.llong += (LL) (((signre >> 30) + k) & 2) << 62;
    pco.llong += (LL) (k + (k & 1)) << 62;

    /* вычисление exm=sinh(im) и exp=cosh(im) */
    pco.llong += (LL) signim << 32;    /* cos(re) with sinh(im)'s sign */
    y = K256ONLN2 * (DB) absim.value;

    if (absim.value > (3 * FL_MAX_LOG)) {
...
//#if __iset__ >= 6
        ...
        exp.value = exp.value * yp + exm.value * ym; /* cosh(im) */
#endif /* __iset__ < 6 */
        if (absim.int0 < 0x3f99999a) {   /* |im| < 1.2 */
            y = (DB) absim.value;
            yp = y * y;
#if __iset__ >= 6
            ly.value = y;
        ...
        else exm.value = y;                              /* sinh(im) */
    }
    __real__ res = (FL) (exp.value * psi.value);   /* sin(re)*cosh(im) */
    __imag__ res = (FL) (exm.value * pco.value);   /* cos(re)*sinh(im) */
    return res;
}
