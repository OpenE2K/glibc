...
//FIXME: почему-то с включением приведения через fma начинают дольше работать все ветки.
// Проверить потом на 24 ветке.
//#if __iset__ >= 6
#if 0
        lres.value = t0;
        lres1.value = X;
...
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                        %
%  ПРОГРАММА  ПРИВЕДЕНИЯ  АРГУМЕНТА ТРИГОНОМЕТРИЧЕСКИХ   %
%  ФУНКЦИЙ  К  ДИАПАЗОНУ  ОТ  -PI/4  ДО  PI/4 .          %
%                                                        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
...
static const unsigned int tbl_rem_pio2[140] = {
 ...
 /*
 /   x - исходный аргумент
 /   lo - указатель на младшую часть возвращаемого результата;
 /
 /   nseg - указатель на целое, содержащее 2 младших разряда
