...
%  ГИПЕРБОЛИЧЕСКИЙ' КОМПЛЕКСНОГО АРГУМЕНТА. НЕДОПУСТИМЫМ %
%  ЗНАЧЕНИЕМ АРГУМЕНТА ЯВЛЯЕТСЯ IM > 710.4758600739439771%
%                                                        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
...
__csinh (__complex__ double x)
{
    ...
    if (__glibc_unlikely (absre.llong >= 0x7ff0000000000000LL)) { /* |re| == Inf or Nan */
        if (absre.llong == 0x7ff0000000000000LL) { /* |re| == Inf */
            ...
            if (absim.llong == 0) {
                ...
                sinix.llong += 0x7ff0000000000000LL;
                cosix.llong += 0x7ff0000000000000LL + signre;
    ...
    if (__glibc_unlikely (absim.llong >= 0x7ff0000000000000LL)) { /* |im| == Inf or Nan */
        absim.llong = signre;
        __real__ res = (absre.llong == 0) ? absim.value : NaN;
        __imag__ res = NaN;
        return res;
    }

    /* вычисление si=sin(im) и co=cos(im) */
    darg1.db = im * TWOONPI;
...
#pragma asm_inline
    ...
    /* вычисление exm=sinh(re) и exp=cosh(re) */
    co.ll += signre;    /* cos(im) with sinh(re)'s sign */
...
#pragma asm_inline
    __asm ("fdtoifd 0x3,%1,%0" : "=r" (y): "r" (y));
        ...
        if (__glibc_likely (k >= -1021 && k <= 1024)) {
            ...
            __real__ res = exm.value * co.db;   /* cos(im)*sinh(re) */
            __imag__ res = exp.value * si.db;   /* sin(im)*cosh(re) */
            return res;
        }

        if (k > 1024) { /* для предотвращения потери точности при очень маленьких sin(im) */
            co.db *= DVAIN1022;
            si.db *= DVAIN1022;
            k -= 1022;
        }
        ...
        co.db = exp.value * co.db;   /* cos(im)*sinh(re) */
        si.db = exp.value * si.db;   /* sin(im)*cosh(re) */
    }
