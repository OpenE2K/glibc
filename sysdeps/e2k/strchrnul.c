 ...
 *
 * @(#) $Id: strchrnul.c 6783 2026-01-27 15:09:02Z malakhov $
 */
...
STRCHRNUL (const char *str, int c_in)
{
  ...
  if ((mask0 | mask) != 0) {
    ...
    __CTZB (mask, mask);
    __CTZB (mask0, mask0);
    return (mask0 < mask) ? (char *) (longword_ptr - 1) + mask0 : (char *) (longword_ptr - 1) + mask;
  }
...
#define __CMP0(mask, x) /* zero bytes -> bit mask */                      \
  ...
  if ((mask0 | mask) != 0) {
    ...
    return (mask0 < mask) ? (char *) str + mask0 : (char *) str + mask;
  }
  qword_ptr += 2;
...
#pragma loop count (100)
  ...
  if ((mask0 | mask) != 0) {
    mask = __builtin_ctz (mask);
    mask0 = __builtin_ctz (mask0);
    return (mask0 < mask) ? (char *) (qword_ptr - 2) + mask0 : (char *) (qword_ptr - 2) + mask;
  }
  mask = __builtin_e2k_qpsgn2mskb (qword1);
  mask0 = __builtin_e2k_qpsgn2mskb (qword3);
  mask = __builtin_ctz (mask);
  mask0 = __builtin_ctz (mask0);
  return (mask0 < mask) ? (char *) (qword_ptr - 1) + mask0 : (char *) (qword_ptr - 1) + mask;
