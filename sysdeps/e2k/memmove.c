...
#if __iset__ < 4 || defined (__elbrus_1cplus__) || defined (__elbrus_2c3__)
#define WC_LIMIT SIZE_L2 /* L3 кэша нет */
#else /* __iset__ < 4 || defined (__elbrus_1cplus__) || defined (__elbrus_2c3__) */
#define WC_LIMIT SIZE_L3
#endif /* __iset__ < 4 || defined (__elbrus_1cplus__) || defined (__elbrus_2c3__) */
...
#pragma loop count (1000)
        ...
        if (len <= 16) {
            if (__builtin_expect (E2K_BYTES_FROM_ALIGN (qsp, 4096) > 4080, 0)) { /* closely to page border */
                ...
                qsrc = __builtin_e2k_ld_128_sm (qsp, len - 16);
                __builtin_pstoremas_128v (qsrc, (char *) qdp - (16 - len), 0xffff << (16 - len), __LCC_MAS_SPEC, __LCC_CHAN_ANY);
            }
            else {
                qsrc = __builtin_e2k_ld_128_sm (qsp, 0);
                __builtin_pstoremas_128v (qsrc, qdp, 0xffff >> (16 - len), __LCC_MAS_SPEC, __LCC_CHAN_ANY);
            }
            RETURN (dst);
        }
...
#pragma loop count (1000)
        ...
        if (len <= 16) {
            if (__builtin_expect (E2K_BYTES_FROM_ALIGN (qsp, 4096) < 16, 0)) { /* closely to page border */
                ...
                qsrc = __builtin_e2k_ld_128_sm (qsp, -len);
                __builtin_pstoremas_128v (qsrc, (char *) qdp - len, 0xffff >> (16 - len), __LCC_MAS_SPEC, __LCC_CHAN_ANY);
            }
            else {
                qsrc = __builtin_e2k_ld_128_sm (qsp, -16);
                __builtin_pstoremas_128v (qsrc, qdp - 1, 0xffff << (16 - len), __LCC_MAS_SPEC, __LCC_CHAN_ANY);
            }
            RETURN (dst);
        }
