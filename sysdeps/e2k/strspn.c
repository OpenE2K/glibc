 ...
 *
 * @(#) $Id: strspn.c 2101 2014-05-13 11:24:32Z vlog $
 */
...
strspn (s, accept)
     ...
     const char *accept;
{
  unsigned long long table[4], table0 = 0, table1 = 0, table2 = 0, table3 = 0;
  ...
  if (len2 <= 8) { /* длина accept <= 8 */

    unsigned long long longword2, mask;
    const unsigned long long *longword_ptr2 = E2K_ALIGN_PTR_BACK (accept, 8);
    size_t align2 = E2K_BYTES_FROM_ALIGN (accept, 8);
...
#pragma loop count (15) /* 1 такт, без apb */
  ...
  if (len2 <= 16) { /* длина accept <= 16 */

    __v2di qword2, qspec, qcharmask;
    const __v2di qzero = __builtin_e2k_qppackdl (0, 0);

    if (__builtin_expect ((E2K_BYTES_FROM_ALIGN (accept, 4096) > 4080), 0)) { /* closely to page border */
      /* qword load is unaligned, till the string end */
      qword2 = ((__v2di *) ((const char *) accept - (16 - len2)))[0];
...
#pragma loop count (15)
  for (i = 0; i < len2; i++) { /* заполняем таблицу символами из accept */
    spec = ((unsigned char *) accept)[i];
    val = 1LL << spec;
    if (spec & 0x80) {
...
#pragma loop count (10) /* 1 такт, без apb */
  while (1) { /* обрабатываем по 1 байту из s */
    spec = *((unsigned char *) p);
    p++;
    if ((*((unsigned long long *) ((char *) table + ((spec >> 3) & 0x18))) & (1LL << spec)) == 0)
      break;
  }
  return p - 1 - s;
}
