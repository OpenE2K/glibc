 ...
 * Updated 8/13/2002 to make f_sin, f_cos, and f_sincos agree.
 *
 ...
 *
 *    Note: for x close to n*pi/2, special treatment is need for either sin or cos:
 ...
 *
 * 3. For x >= 8.0, use kernel function __rem_pio2 to perform argument reduction and
 *    call __k_sincos_ to compute sin and cos.
 *
 * kernel function:
...
/* __k_sincos_(double x[],double *c)
 * Accuracy Table look-up algorithm by K.C. Ng, May, 1995.
 * Assume |x| <~ pi/4.
 * Return sin(x[0]+x[1]) with *c = cos(x[0]+x[1])
 *
