...
static void* __malloc_alloc(size_t nb, mstate av)
{
...
#else /* __PROTECTED__ */
	return malloc_internal_1 (nb - MALLOC_ALIGN_MASK, 0);
#endif	/* __PROTECTED__ */
    }
...
#else /* __PROTECTED__ */
	/* Additional size for pointer to next mmaped_region. */
	size = (nb + sizeof(mprotptr) + (sizeof(size_t)) + MALLOC_ALIGN_MASK
		+ pagemask) & ~pagemask;
#endif	/* __PROTECTED__ */
	...
	if ((unsigned long)(size) > (unsigned long)(nb)) {
		...
		if (front_misalign > 0) {
...
#ifdef __PROTECTED__
		    ((mprotptr)p)->fd = av->mmaped_region;
		    av->mmaped_region = (mprotptr) p;
		    p = (void *)p + sizeof(mprotptr);
		    correction += sizeof(mprotptr);
#endif
		    ...
		    av->mmaped_region = (mprotptr) p;
		    p = (void *)p + sizeof(mprotptr);
		    p->prev_size = sizeof(mprotptr);
		    size -= sizeof(mprotptr);
#endif
...
#ifdef __PROTECTED__
		((mprotptr)fst_brk)->fd = av->mmaped_region;
		av->mmaped_region = (mprotptr) fst_brk;
		fst_brk = ((char *)fst_brk) + sizeof(mprotptr);
		size -= sizeof(mprotptr);
#endif
...
#else /* __PROTECTED__ */
			free_internal(chunk2mem(old_top));
#endif /* __PROTECTED__ */
...
#else
static void *
malloc_internal_1 (size_t bytes, int lock)
#endif
{
    mstate av;
...
use_top:
...
#ifdef __PROTECTED__
    /* If no space in top, try to free descriptors in list */
    if (av->list_descriptors_to_free_size > 0) {
	...
	for (idx = 0; idx < av->list_descriptors_to_free_size; idx++) {
		retval = (av->list_descriptors_to_free)[idx];
		retval = pmem2mem(retval, av);
		free_internal(retval);
	}
	av->list_descriptors_to_free_size = 0;

        /* After free try to malloc one more time */
	retval = malloc_internal_1(bytes, 0);
	if (retval != NULL)
	    goto DONE;
    }
#endif
...
static void* malloc_1(size_t bytes, int lock)
{
    ...
    if (bytes > 0x7fffffff) {
	if (IF_PM_DBG_MODE_CHECK) {
		printf("ERROR: trying to malloc %zd (0x%zx) bytes\n", bytes, bytes);
		printf("malloc cannot allocate over 2**31 bytes (2Gb) in protected mode\n");
	}
	__set_errno(ENOMEM);
	return NULL;
    }
    ...
    retval = malloc_internal_1(alloc_bytes, lock);
    if (retval == NULL)
	return NULL;
