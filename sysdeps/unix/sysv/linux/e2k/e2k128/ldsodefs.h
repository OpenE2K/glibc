...
#define DL_CALL_DT_INIT(map, start, argc, argv, env)		\
  ...
  /* Note that these idiots in `call_init ()' invoked us with a	\
    ...
    dl_init_t init = (dl_init_t) get_pl (map, start - map->l_addr);	\
    init (argc, argv, env);					\
  }
	...
	unsigned long addend, unsigned long size)
{
  ...
  if (size == 0)
    {
      /* Treat 0 SIZE by analogy with legacy `__selfinit ()'. This usually
	 means that the symbol's value is not known during the compilation
	 as in case when it just marks the start of an array linked from
	 several sections belonging to different input files. Because there's
	 no realistic way to determine the actual size at link-time too (as
	 the linker is unaware of the implied relation between "starting" and
	 "trailing" input symbols), one has to extend the array from its start
	 to the end of GD.  */
      if (__builtin_e2k_get_ap_size (map->l_gd) < off)
	/* This is something inappropriate.  */
	return NULL;

      size = __builtin_e2k_get_ap_size (map->l_gd) - off;
    }
...
(* get_pl (struct link_map *map, unsigned long off)) (void)
{
  ...
  return map->get_pl (get_offset (map, off));
}
...
#if defined FROM_DL_RELOC_STATIC_PIE
  void (* pl) (void) = NULL;
  /* Hopefully "+r" constraint will prevent the compiler from optimizing out
     the above initialization of PL ensuring that its hi 64 bits are zeroed
     out on elbrus-v{X<6} (Bug #148194).  */
  _Pragma ("asm_inline") __asm__ ("getpl %1, %0" : "+r" (pl) : "r" (off));
  return pl;
#else /* ! defined FROM_DL_RELOC_STATIC_PIE  */
  return map->get_pl (off);
#endif /* ! defined FROM_DL_RELOC_STATIC_PIE  */
}
