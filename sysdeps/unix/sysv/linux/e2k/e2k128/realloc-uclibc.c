...
void* realloc(void* oldmem, size_t bytes)
{
	...
	/*
	 * Note the extra (sizeof(size_t)) overhead
	 * and pointer to next mmaped_region.
	 */
	newsize = (nb + offset + (sizeof(size_t) + sizeof(mprotptr)) + pagemask)
								& ~pagemask;
#endif /* __PROTECTED__ */
...
#ifdef __PROTECTED__
	    /* Add new region */
	    ((mprotptr)cp)->fd = av->mmaped_region;
...
#ifdef __PROTECTED__
	/* If mremap() returns error, we need to add old region back,
	   because it whould be deleted by free() */
	((mprotptr)((char *)oldp - sizeof(mprotptr)))->fd = av->mmaped_region;
	av->mmaped_region = (mprotptr)((char *)oldp - offset);
#endif /* __PROTECTED__ */
