...
/* FIXME: this include has resulted in repetitive typedefs, however, now that
   it's commented out other malloc-related types are missing. Find out how
   they are distributed across uclibc-ng source tree.  */
// #include <malloc.h>


/* Stupidly borrowed from `uclibc-ng's `include/malloc.h' which is not included
   one line above.  */
struct mallinfo {
...
/* These extern declarations and `libc_hidden_proto's matching the underlying
   `libc_hidden_def's in `{memalign,mallinfo}-uclibc.c' source files are
   borrowed from `uclibc-ng/include/malloc.h'.  */
...
#endif
	;
#define __MALLOC_LOCK		__libc_lock_lock (__malloc_lock)
#define __MALLOC_UNLOCK		__libc_lock_unlock (__malloc_lock)
...
# else /* __PROTECTED__ */
/*
  We should not use brk/sbrk for protected mode to obtain memory from
  the system, otherwise user can access all heap memory with brk/sbrk
  system call.
*/
#  define MORECORE(x) (-1)
# endif /* __PROTECTED__ */
...
/* This is typically defined to 0 for architectures using MMU to which
   e2k128 belongs even though glibc doesn't provide `__ARCH_USE_MMU__'
   for it.  */
#if 1 /* def __ARCH_USE_MMU__  */
...
#ifdef __PROTECTED__
/*
  Structure for build mmaped_region list.
*/
struct mem_prot {
  struct mem_prot *fd;
};

typedef struct mem_prot* mprotptr;
...
struct malloc_state {
  ...
  void **list_descriptors_to_free;
  size_t list_descriptors_to_free_size;
  size_t list_descriptors_to_free_size_max;
  unsigned int pm_mm_mode; /* PM memory management (debug) mode (see in <malloc.h>) */
#endif /* __PROTECTED__ */
};
...
#ifdef __PROTECTED__
void *mem2pmem(void *, size_t) attribute_hidden;
void *pmem2mem(void *, mstate) attribute_hidden;
void delete_region(void *, mstate) attribute_hidden;
static void *malloc_internal_1(size_t, int);
static void *malloc_1(size_t, int);
static void free_internal(void *);
static void free_1(void *, int);
static int clean_descriptors(void *, long, unsigned long);

/* Enable check for dangling descriptors: */
#define PM_MM_MODE_CHECK_DANGLING_POINTERS   1 /* run clean_descriptors procedure */
/* Zeroing freed descriptor contents / no check for dangling descriptors: */
#define PM_MM_MODE_ZEROING_FREED_POINTERS    2
/* Reusing freed descriptors after freed descriptor list is full: */
#define PM_MM_MODE_EMPTYING_FREED_POINTERS   4
