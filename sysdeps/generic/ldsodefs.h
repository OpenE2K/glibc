...
/* There's no point in `dl_relocate_ld (map)' here as it's known to
   unconditionally return false in PM.  */
# define D_PTR(map, i) ((map)->l_gd + get_offset (map, (map)->i->d_un.d_ptr))

#else /* ! defined __ptr128__  */

# define D_PTR(map, i) \
  (void *) ((map)->i->d_un.d_ptr + (dl_relocate_ld (map) ? 0 : (map)->l_addr))
...
# define DL_NNS 1
  ...
  EXTERN struct link_namespaces
  {
	...
	const struct link_map *map;
      ...
      size_t size;
      size_t n_elements;
      /* Renamed from `free' to `free_m' for the sake of compilation with `LCC
	 -O0' requiring `free(m)' to be a macro with rtld-malloc.h included
	 because of unworkable inline functions.  */
      void (*free_m) (void *);
