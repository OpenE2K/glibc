...
elf_dynamic_do_Rel (struct link_map *map,
		    const void *reladdr, ElfW(Addr) relsize,
		    __typeof (((ElfW(Dyn) *) 0)->d_un.d_val) nrelative,
		    int lazy, int skip_ifunc, struct r_scope_elem *scope[],
                    const char *strtab)
{
...
# endif
	  elf_machine_lazy_rel (map, l_addr, r, skip_ifunc, scope, strtab);

# ifdef ELF_MACHINE_IRELATIVE
      if (r2 != NULL)
	for (; r2 <= end2; ++r2)
	  if (ELFW(R_TYPE) (r2->r_info) == ELF_MACHINE_IRELATIVE)
	    elf_machine_lazy_rel (map, l_addr, r2, skip_ifunc, scope, strtab);
# endif
    }
  else
#endif
