...
#else /* defined __ptr128__  */
	l->l_data_start = l->l_data_end = l->l_text_start = l->l_text_end = 0;
#endif /* defined __ptr128__  */
	goto lose;
      }
  }
...
#if defined __ptr128__
  /* Let it initialize itself as soon as possible. With `__selfinit ()'less
     implementation available this is possible only after getting access to
     `.dynamic'. But may not it turn out to be too late ? ? ?  */
  if (l->l_info[DT_E2K_NO_SELFINIT - DT_LOPROC + DT_NUM] == NULL)
    l->selfinit ();
