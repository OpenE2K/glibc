...
struct link_map
{
    ...
    /* TODO: currently l_contiguous is stupidly set to 0 in PM, but in future
       one could introduce distinct `l_{text,data}_contiguous' to  eliminate
       the need for iterating through Program Headers if the address belongs
       to `[l_{text,data}_start, l_{text,data}_end)' interval.  */
