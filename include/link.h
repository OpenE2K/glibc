...
struct link_map
  {
    ...
    /* TODO: currently I avoid evaluating and using any analogs of l_contiguous
       in PM, but in future one could introduce `l_{text,data}_contiguous' to
       eliminate the need for iterating through Program Headers if the address
       belongs to `[l_{text,data}_start, l_{text,data}_end)' interval.  */
#if ! defined __ptr128__
...
#include <link_map.h>
    ...
    /* Start and finish of data memory map for this object.  l_data_start
       need not be the same as l_addr.  */
    ElfW(Addr) l_data_start, l_data_end;
    /* The same for the executable part of the mapping.  */
...
# endif
...
#if defined __ptr128__
    void *l_gd;
    /* This field contains 0 if it hasn't been determined whether the LINK_MAP
       is packed or not, 1 if it is not and 2 if it is.  */
    enum
      {
	PACKED_UNKNOWN,
	PACKED_LEGACY,
    ...
    struct range
    {
	 ...
	 code generated by e2k-linux-ld? Definitely yes because one needs to
	 evaluate the actual runtime address of a function to fetch the
	 right PL from DT_E2K_EXPORT_PL. They are also needed for dladdr (addr_
	 in_text_segment).  */
      int in_cud;
    } ranges[8];
#endif /* defined __ptr128__  */
